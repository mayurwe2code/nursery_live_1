...
export function user_signup(req, res) {
  ...
  if (req.body.email != "" && req.body.password != "") {
    ...
    let user_name = req.body.name.trim()
        ...
        (err, rows) => {
          if (err) {
            ...
            if (rows != "") {
              ...
              connection.query('INSERT INTO `user_auth_by_otp` (`email`, `otp`, `user_password`,`user_name`) VALUES ("' + u_email + '","' + OTP + '","' + u_password + '","' + user_name + '")', (err, rows, fields) => {
                if (err) {
      ...
      if (err) {
        ...
        if (rows != "") {
          if (user_email == rows[0].email && user_otp == rows[0].otp) {
            ...
            connection.query("insert into user  ( `email`,`password`,`first_name`) VALUES('" + user_email + "','" + rows[0].user_password + "','" + rows[0].user_name + "') ",
              (err, rows) => {
                if (err) {
                  console.log(err)
